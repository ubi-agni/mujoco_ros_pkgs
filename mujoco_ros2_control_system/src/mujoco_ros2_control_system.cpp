...
bool MujocoRos2System::initSim(rclcpp_lifecycle::LifecycleNode::SharedPtr &model_nh,
			...
			if (joint_info.command_interfaces[j].name == "position") {
				...
				if (act_idx == -1) {
					RCLCPP_WARN(this->nh_->get_logger(),
					            "Could not find a position actuator with name %s in the mujoco xml file.", act_name.c_str());
...
MujocoRos2System::prepare_command_mode_switch(const std::vector<std::string> &start_interfaces,
...
// Documentation Inherited
hardware_interface::return_type MujocoRos2System::read(const rclcpp::Time &time, const rclcpp::Duration & /* period */)
{
		...
		for (uint i = 0; i < this->dataPtr_->joints_.size(); i++) {
			...
			if (this->dataPtr_->joints_[i].act_posidx != -1) {
				if (this->dataPtr_->joints_[i].joint_control_method & ControlMethod_::POSITION) {
					this->dataPtr_->d_->ctrl[this->dataPtr_->joints_[i].act_posidx] =
					    this->dataPtr_->joints_[i].joint_position_cmd;
				} else {
			...
			if (this->dataPtr_->joints_[i].act_velidx != -1) {
				if (this->dataPtr_->joints_[i].joint_control_method & ControlMethod_::VELOCITY) {
					this->dataPtr_->d_->ctrl[this->dataPtr_->joints_[i].act_velidx] =
					    this->dataPtr_->joints_[i].joint_velocity_cmd;
				} else {
			...
			if (this->dataPtr_->joints_[i].act_effidx != -1) {
				if (this->dataPtr_->joints_[i].joint_control_method & ControlMethod_::EFFORT) {
					this->dataPtr_->d_->ctrl[this->dataPtr_->joints_[i].act_effidx] =
					    this->dataPtr_->joints_[i].joint_effort_cmd;
				} else {
...
PLUGINLIB_EXPORT_CLASS(mujoco_ros2_control::MujocoRos2System, mujoco_ros2_control::MujocoRos2SystemInterface)
