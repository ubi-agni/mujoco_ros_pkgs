...
Mesh ``filename`` URIs are normalized before MuJoCo parse. MuJoCo 3.3.5 also
does not load ``.glb``, and only loads **binary** STL (and OBJ), not ASCII
STL -- both are handled as an explicit pipeline before the mesh reaches
MuJoCo:

.. figure:: images/mesh-pipeline.svg
   :alt: A URDF mesh URI is resolved by kind, then GLB visuals are rewritten in memory to STL, then ASCII STL falls back to a collision OBJ or in-process conversion, before the result is supplied to MuJoCo through mjVFS. Each stage fails loudly rather than silently substituting a default.
   :width: 100%

   Every stage either passes the mesh through unchanged or fails loudly -- nothing is silently substituted, and the on-disk URDF and source mesh are never modified.
...
A ``<joint>`` element may carry the ROS ``<friction>`` extension (``ds``,
``vd``, ``k1``, ``k2``, ``k3``). MuJoCo's URDF importer ignores that tag, so
the converter parses it itself (``mujoco_ros::ParseUrdfJointFriction``) and
applies it to the joint's **child link** geoms
(``mujoco_ros::ApplyUrdfJointFriction``):

* ``ds`` (dry friction coefficient) scales MuJoCo's conventional
  slide/spin/roll ratio: ``fric = [ds, ds*0.005, ds*0.0001]``.
* ``vd``/``k1``/``k2``/``k3`` are decay-rate/stiffness terms of the ROS
  bristle contact model. MuJoCo's friction cone has no analog for them, so
  they are ignored.
* A ``<friction>`` tag without a usable ``ds`` is warned and skipped; the
