...
void PlatformUIAdapter::OnKey(int key, int /*scancode*/, int act)
{
	...
	state_.key     = mj_key;
	state_.keytime = std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch()).count();
...
{
	// translate API-specific button code
...
void PlatformUIAdapter::OnMouseMove(double /*x*/, double /*y*/)
{
	...
	if (!state_.left && !state_.middle && !state_.right) {
		return;
	}
