...
void UpdateProfiler(mujoco_ros::Viewer *viewer, const mjModel *m, const mjData *d)
{
	...
	for (int i = 0; i < viewer->figconstraint.linepnt[0]; i++) {
		// x
		viewer->figconstraint.linedata[0][2 * i] = i;
		viewer->figconstraint.linedata[1][2 * i] = i;
		viewer->figconstraint.linedata[2][2 * i] = i;
		viewer->figconstraint.linedata[3][2 * i] = i;
		viewer->figconstraint.linedata[4][2 * i] = i;

		// y
		viewer->figconstraint.linedata[0][2 * i + 1] = d->nefc;
		viewer->figconstraint.linedata[1][2 * i + 1] = d->solver[i].nactive;
		viewer->figconstraint.linedata[2][2 * i + 1] = d->solver[i].nchange;
		viewer->figconstraint.linedata[3][2 * i + 1] = d->solver[i].neval;
		viewer->figconstraint.linedata[4][2 * i + 1] = d->solver[i].nupdate;
	}
	...
	for (int i = 0; i < viewer->figcost.linepnt[0]; i++) {
		// x
		viewer->figcost.linedata[0][2 * i] = i;
		viewer->figcost.linedata[1][2 * i] = i;
		viewer->figcost.linedata[2][2 * i] = i;
                    ...
                    char (&content)[mujoco_ros::Viewer::kMaxFilenameLength])
{
	...
	mju::sprintf_arr(content, "%-9.3f\n%d  (%d con)\nb: %d%% m: %d%%\n%.3f\n%.1f  (%d it)\n%s\n%.2g of %s", d->time,
	                 ...
	                 solerr, d->solver_iter, fps, d->maxuse_arena / (double)(d->nstack * sizeof(mjtNum)),
...
void MakeJointSection(mujoco_ros::Viewer *viewer, int oldstate)
{
	...
	int itemcnt = 0;
	for (int i = 0; i < viewer->jnt_type_.size() && itemcnt < mjMAXUIITEM; i++)
		if ((viewer->jnt_type_[i] == mjJNT_HINGE || viewer->jnt_type_[i] == mjJNT_SLIDE)) {
			...
			if (viewer->fully_managed_) {
				defSlider[0].pdata = &viewer->d_->qpos[viewer->m_->jnt_qposadr[i]];
			} else {
				defSlider[0].pdata = &viewer->qpos_[viewer->jnt_qposadr_[i]];
			}
...
void MakeControlSection(mujoco_ros::Viewer *viewer, int oldstate)
{
	...
	int itemcnt = 1;
	for (int i = 0; i < viewer->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++) {
		...
		switch (state->key) {
			...
			case mjKEY_PAGE_UP: // select parent body
				if ((viewer->m_ || !viewer->fully_managed_) && viewer->pert.select > 0) {
					viewer->pert.select     = viewer->body_parentid_[viewer->pert.select];
...
Viewer::Viewer(std::unique_ptr<PlatformUIAdapter> platform_ui, MujocoEnv *env, bool fully_managed)
...
#undef X
	...
	if (pending_.load_key) {
		int i = this->key;
		// TODO(dleins): Look up Keypoint loading in MuJoCo ROS
		d_->time = m_->key_time[i]; // Probalbly not a good idea...
...
void Viewer::LoadOnRenderThread()
{
	...
	std::memcpy(jnt_type_.data(), this->mnew_->jnt_type, sizeof(this->mnew_->jnt_type[0]) * this->mnew_->njnt);
	jnt_type_.shrink_to_fit();

	jnt_group_.resize(this->mnew_->njnt);
	std::memcpy(jnt_group_.data(), this->mnew_->jnt_group, sizeof(this->mnew_->jnt_group[0]) * this->mnew_->njnt);
	jnt_group_.shrink_to_fit();

	jnt_qposadr_.resize(this->mnew_->njnt);
	std::memcpy(jnt_qposadr_.data(), this->mnew_->jnt_qposadr, sizeof(this->mnew_->jnt_qposadr[0]) * this->mnew_->njnt);
	...
	std::memcpy(qpos_.data(), this->mnew_->qpos0, sizeof(this->mnew_->qpos0[0]) * this->mnew_->nq);
	qpos_.shrink_to_fit();
	qpos_prev_ = qpos_;
...
void Viewer::Render()
{
	...
	if (desired_real_time != 100.0f || misaligned) {
		...
		std::snprintf(rtlabel + labelsize, sizeof(rtlabel) - labelsize, " (%-4.1f%%)", actual_real_time);
	}
	...
	// Show help
	if (this->help) {
		mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect, help_title, help_content, &this->platform_ui->mjr_context());
	}
	...
	while (!this->platform_ui->ShouldCloseWindow() && !this->exit_request.load()) {
		{
			...
			// Update scene, doing a full sync
			{
