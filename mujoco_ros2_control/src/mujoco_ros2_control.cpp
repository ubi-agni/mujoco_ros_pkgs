...
bool MujocoRos2ControlPlugin::Load(const mjModel *model, mjData *data)
{
	...
	RCLCPP_INFO_STREAM(dataPtr_->node_->get_logger(),
	...
	RCLCPP_INFO(dataPtr_->node_->get_logger(), "robot_description_node fully qualified name: %s",
	...
	RCLCPP_INFO_STREAM(dataPtr_->node_->get_logger(),
	                   "Fully qualified mujoco_ros2_control node name: "
	...
	try {
		...
		RCLCPP_ERROR_STREAM(this->dataPtr_->node_->get_logger(),
	...
	try {
		...
		RCLCPP_ERROR(this->dataPtr_->node_->get_logger(), "Error initializing URDF to resource manager!");
	}
	try {
		this->dataPtr_->robot_hw_sim_loader_.reset(
		    new pluginlib::ClassLoader<mujoco_ros2_control::MujocoRos2SystemInterface>(
		        "mujoco_ros2_control", "mujoco_ros2_control::MujocoRos2SystemInterface"));
	} catch (pluginlib::LibraryLoadException &ex) {
		RCLCPP_ERROR(this->dataPtr_->node_->get_logger(), "Failed to create robot simulation interface loader: %s ",
		             ex.what());
		return false;
	}
	for (unsigned int i = 0; i < control_hardware_info.size(); ++i) {
		...
		RCLCPP_DEBUG(this->dataPtr_->node_->get_logger(), "Load hardware interface %s ...",
		...
		try {
			...
			RCLCPP_ERROR(this->dataPtr_->node_->get_logger(), "The plugin failed to load for some reason. Error: %s\n",
			             ex.what());
			continue;
		}
		if (!mujocoRos2System->initSim(this->dataPtr_->node_, control_hardware_info[i], model, data,
		                               this->dataPtr_->update_rate)) {
			RCLCPP_FATAL(this->dataPtr_->node_->get_logger(), "Could not initialize robot simulation interface");
			return false;
		}
		RCLCPP_DEBUG(this->dataPtr_->node_->get_logger(), "Initialized robot simulation interface %s!",
		             robot_hw_sim_type_str_.c_str());

		RCLCPP_DEBUG_STREAM(this->dataPtr_->node_->get_logger(),
		...
		RCLCPP_DEBUG_STREAM(this->dataPtr_->node_->get_logger(),
		                    "resource-manager system comp size: " << resource_manager_->system_components_size());
		RCLCPP_DEBUG(this->dataPtr_->node_->get_logger(), "Setting state of %s to active",
		             control_hardware_info[i].name.c_str());
	...
	RCLCPP_WARN_STREAM(this->dataPtr_->node_->get_logger(),
	                   this->dataPtr_->controller_manager_->get_fully_qualified_name()
