...
mujoco_ros::CallbackReturn DummyRos2Plugin::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
	...
	declare_parameter_if_not_declared(this->get_node()->get_node_parameters_interface(), "parallel_name",
	                                  rclcpp::ParameterValue("parallel_node_name"));
	declare_parameter_if_not_declared(this->get_node()->get_node_parameters_interface(), "child_name",
...
bool DummyRos2Plugin::Load(const mjModel *m, mjData *d)
{
	...
	RCLCPP_INFO_STREAM(get_my_logger(),
	                   "Namespace: " << node_handle->get_namespace() << "; Fully qualified name: "
	                                 << node_handle->get_node_base_interface()->get_fully_qualified_name());
	...
	RCLCPP_INFO_STREAM(rclcpp::get_logger("ParallelNode"),
	                   "Namespace: " << parallel_node_->get_namespace() << "; Fully qualified name: "
	                                 << parallel_node_->get_node_base_interface()->get_fully_qualified_name());
	...
	const auto child_options =
	    rclcpp::NodeOptions().arguments({ "--ros-args", "--remap", child_name + ":__node:=" + child_name });

	// This is in practice more complex, because we have to check first if the node is in a namespace itself and then we
	// add the node name plus the child name. But since we know how things are set up in this example, we just do it
	// quick and dirty
	...
	RCLCPP_INFO_STREAM(rclcpp::get_logger("ChildNode"),
	                   "Namespace: " << child_node_->get_namespace() << "; Fully qualified name: "
	                                 << child_node_->get_node_base_interface()->get_fully_qualified_name());
